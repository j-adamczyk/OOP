...
{
    /**
     * Parses single record, creating ready, filled Entry from String.
     * @param record single record to be parsed
     * @return Entry object representing parsed record
     */
    public Entry parseSingleRecord(String record)
    {
        ...
        entry.key = getKey(record);
        entry.crossref = getCrossref(record);
    ...
    /**
     * Gets empty Entry object of type passed as String. If no such type is defined (e. g. there is no Factory for such type) it returns null.
     * @param entryType type of entry to be created (determines Factory class used)
     * @return empty Entry of given type or null (if type is not defined)
     */
     ...
     * @param entry Entry object which values will be filled
     * @param record String record which Entry represents
     * @return Entry will filled value fields
     */
    public Entry fillFields (Entry entry, String record)
    {
        ...
        for (int i=0; i<fields.size(); i+=2)
        {
            ...
            if(entry.requiredAttributes.get(field)!=null)
                entry.requiredAttributes.put(field,value);
            else if (entry.optionalAttributes.get(field)!=null)
                entry.optionalAttributes.put(field,value);
        }
