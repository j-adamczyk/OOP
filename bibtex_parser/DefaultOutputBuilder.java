...
public class DefaultOutputBuilder implements IOutputBuilder
{
    ...
    public String buildOutput(List<Entry> entries, String sign)
    {
        ...
        for (Entry entry : entries)
        {
            String tmp = "";
            int entire_length = 0;
            for (Map.Entry<String,String> iter : entry.requiredAttributes.entrySet())
                if (iter.getValue().length() > entire_length)
                    entire_length = iter.getValue().length()+1;

            for (Map.Entry<String,String> iter : entry.optionalAttributes.entrySet())
                if (iter.getValue().length() > entire_length)
                    entire_length = iter.getValue().length()+1;
            ...
            tmp = sign + entry.recordType + " (" + entry.key + ")";
            ...
            for (Map.Entry<String,String> iter : entry.requiredAttributes.entrySet())
            {
                if (iter.getKey().equals("author") && iter.getValue().equals("") && entry.requiredAttributes.containsKey("editor")
                        && !entry.requiredAttributes.get("editor").equals(""))
                    continue;

                if (iter.getKey().equals("editor") && iter.getValue().equals("") && entry.requiredAttributes.containsKey("author")
                        && !entry.requiredAttributes.get("author").equals(""))
                    continue;
