...
{
    /**
     * Performs all substitutions defined in @STRING records and saves result as SubstitutedText in UtilSingleton.
     * Uses other methods in this class, which are this class's utilities.
     */
    public void substitute()
    {
        List<String> records = getRecords(UtilSingleton.getInstance().originalText);
        Map<String,String> substitutions = getSubstitutions(records);

        for (Map.Entry<String,String> iter : substitutions.entrySet())
        {
            UtilSingleton.getInstance().substitutedText = UtilSingleton.getInstance().substitutedText.replace(iter.getKey(),iter.getValue());
            ...
            while (mat.find())
            {
                ...
                UtilSingleton.getInstance().substitutedText = UtilSingleton.getInstance().substitutedText.replace(mat.group(1),tmp);
            }
        }
    }
